...
const remainingWindowDays = (item: ImageImportItem): number | null => {
  if (!item.purge_eligible_at || item.objects_purged_at) return null;
  const deadline = new Date(item.purge_eligible_at).getTime();
      ...
      {items.length === 0 && !loading ? (
        ...
        <List
          ...
          renderItem={(item) => {
            ...
            const windowDays = remainingWindowDays(item);
            ...
            if (restorable) {
              actions.push(
                <Button
                  key="restore"
                  type="link"
                  loading={busyItemIds.includes(item.item_id)}
                  onClick={() => onRestoreItem!(item.item_id)}
                >
                  恢复导入
                </Button>
              );
            ...
            if (abandonable) {
              actions.push(
                <Button
                  ...
                  danger
                  loading={busyItemIds.includes(item.item_id)}
                  onClick={() => onAbandonItem!(item.item_id)}
                >
                  提前放弃
                </Button>
              );

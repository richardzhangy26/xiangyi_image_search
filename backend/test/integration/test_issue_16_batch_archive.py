...
def test_batch_archive_updates_status_time_version_and_audit_atomically(app):
    ...
    assert {
        row.id: (row.vector, row.oss_path, row.preview_oss_path) for row in rows
    } == originals
...
def test_archived_asset_preview_remains_private_and_available(app):
    ...
    class FakeStorage:
        ...
        def sign_download_url(self, path, ttl):
            self.calls.append((path, ttl))
            return 'https://signed.example.test/private-preview'

...
describe('ProductUpload unified management view', () => {
  ...
  const submitNewProduct = async () => {
    fireEvent.click(await screen.findByRole('button', { name: /添加产品/ }));
  ...
  it('restores persisted import status and opens the unresolved task drawer', async () => {
    ...
    // Issue #20+#21 汇合：重试与取消控件合法并存；仍不得出现「放弃」类控件
    expect(screen.queryByRole('button', { name: /放弃/ }))
  ...
  it('defaults to real unassigned assets when there are no products', async () => {
    ...
    expect(screen.queryByRole('button', { name: '关联型号' })).not.toBeInTheDocument();
